	...
	var xe,
		...
		m = function _updateAll() {
			var e = Ve.length,
				...
				n = 50 <= r - u;
			for (
				n && (ze && !Pe && 200 < r - ze && ((ze = 0), v("scrollEnd")), (_e = u), (u = r));
				t < e;
				t++
			)
	...
	var _ =
		((ScrollTrigger.prototype.init = function init(y, x) {
			if (((this.progress = 0), this.vars && this.kill(1), Fe)) {
				var d,
					...
					Z,
					$,
					K = (y = da(F(y) || H(y) || y.nodeType ? { trigger: y } : y, Ue)).horizontal
						? Je
						: Xe,
					Q = y.onUpdate,
					ee = y.toggleClass,
				...
				(se *= 45),
					...
					(me.vars = y),
					(x = x || y.animation),
					(l.tweenScroll = l.tweenScroll || { top: Ma(pe, Xe), left: Ma(pe, Je) }),
					...
					(me.enable = function () {
						$ ||
							...
							x &&
								(x.add
									? xe.delayedCall(0.01, me.refresh) && (O = 0.01) && (T = S = 0)
									: me.refresh()));
					}),
		...
		(ScrollTrigger.register = function register(e) {
			if (
				...
				A() &&
					window.document &&
					((we = window), (Te = document), (x = Te.documentElement), (Se = Te.body)),
				xe &&
					((Ce = xe.utils.toArray),
					(ke = xe.utils.clamp),
