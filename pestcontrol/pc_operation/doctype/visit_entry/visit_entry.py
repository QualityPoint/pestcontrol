...
class VisitEntry(Document):
	...
	def validate_visit_reference_unique(self):
		...
		*scope* of the constraint (it never disables it):
		...
		per_company = frappe.db.get_single_value("Operation Settings", "unique_visit_reference")
	...
	def validate_operation_order(self):
		...
		if order.docstatus != 1:
			frappe.throw(
				_("Operation Order {0} must be submitted.").format(frappe.bold(self.operation_order))
			)
	...
	def validate_amenity_pest_types(self):
		...
		for row in self.amenities:
			if not (row.pest_type or "").strip():
				self._row_throw(
					row,
					_("select at least one Pest Type for amenity {0}.").format(
