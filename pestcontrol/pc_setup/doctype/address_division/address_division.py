...
class AddressDivision(NestedSet):
	...
	def validate_parent_division(self):
		if self.division_type:
			division_type_doc = frappe.get_doc("Address Division Type", self.division_type)
		...
		if self.parent_division:
			...
			if self.division_type:
				...
				if expected_parent_type and parent.division_type != expected_parent_type:
					frappe.throw(
						_(
							"Parent Division must be of Division Type '{0}', but '{1}' has Division Type '{2}'"
						).format(
							expected_parent_type, self.parent_division, parent.division_type or _("none")
						)
					)
...
def add_node():
	...
	is_group = (
		...
		if (
			division_type
			and frappe.db.exists("Address Division Type", {"parent_division_type": division_type})
		)
		else 0
	)
...
def get_children(
	...
	fields = ["name as value", "division_name as title", "is_group", "division_type"]
	...
	members = frappe.get_list(doctype, fields=fields, filters=filters, order_by="name")
