...
frappe.treeview_settings["Address Division"] = {
	get_tree_nodes: "pestcontrol.pc_setup.doctype.address_division.address_division.get_children",
	add_tree_node: "pestcontrol.pc_setup.doctype.address_division.address_division.add_node",
	filters: [
		{
			fieldname: "country",
			fieldtype: "Link",
	...
	toolbar: [
		{
			...
			click: function (node) {
				frappe.call({
					method: "pestcontrol.pc_setup.doctype.address_division.address_division.get_child_node_info",
					args: { parent: node.data.value },
					callback(r) {
						if (!r.message) return;
						const { country, child_division_types } = r.message;

						const division_type_field =
							child_division_types.length === 1
								? {
						...
						d.set_primary_action(__("Create"), function () {
							...
							frappe.call({
								method: "pestcontrol.pc_setup.doctype.address_division.address_division.add_node",
								args: values,
								callback(r) {
									if (!r.exc) {
										frappe.views.trees["Address Division"].tree.load_children(
											node,
											true
										);
									}
