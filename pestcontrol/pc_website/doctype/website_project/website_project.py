...
class WebsiteProject(WebsiteGenerator):
	website = frappe._dict(template="pestcontrol/templates/generators/website_project.html")
	...
	def validate(self):
		...
		if not self.route:
			...
			if title := route_from_article(self):
				self.route = f"project/{self.scrub(title)}"
		super().validate()
	...
	def get_context(self, context):
		...
		context.title = localize(self, "project_name")
